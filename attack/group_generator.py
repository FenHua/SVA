...
class EquallySplitGrouping():
    def __init__(self, divide_number):
        self.length = 0
        self.dim = None
        self.divide_number = divide_number    # partition numbers

    # parameter setting
    def initialize(self, x):
        assert x.size(-1) % self.divide_number == 0, 'frame size: {} not divided evenly by {}'.format(
            x.size(-1),self.divide_number)    # judge whether frame can be evenly divided
        self.length = self.divide_number * self.divide_number * x.size(0)   # block numbers
    ...
    # different perturbations to different blocks
    def apply_group_change(self, x, y):
        # y adversarial noise， x perturbation direction
        ...
        x_t = x.repeat((y.size(0),) + (1,) * len(x.size())) if batch_mode else x.clone()   # uniform dimension size
        # shallow copy
