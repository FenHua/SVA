...
# primary perturbation generator
class TentativePerturbationGenerator():
    def __init__(self, extractors, part_size=100, preprocess=True, device=0):
        ...
        self.part_size = part_size      # block size
        self.preprocess = preprocess    # preprocessing
        self.device = device            # device id

    # parameter settings (Targeted)
    def set_targeted_params(self, target_vid, random_mask=1.):
        self.target = True                    # Targeted
        self.random_mask = random_mask        # mask
        ...
        with torch.no_grad():
            ...
            for extractor in self.extractors:
                outputs = extractor(target_vid)               # output
                self.target_feature.append(outputs[0].view((outputs[0].size(0), -1)))

    # parameter settings(Untargeted)
    def set_untargeted_params(self, ori_video, random_mask=1., translate=0., scale=1.):
        ...
        self.target_feature = []             # features
        with torch.no_grad():
            ori_video = ori_video.clone().cuda(self.device)    # original video
            for extractor in self.extractors:
                outputs = extractor(ori_video)                 # outputs
                output_size = outputs[0].size()                # get output size
                del outputs
                r = torch.randn(output_size, device=self.device) * self.scale + self.translate  # transform
                r = torch.where(r >= 0, r, -r)                 # clip operations
                self.target_feature.append(r.view((output_size[0], -1)))

    # update the specified region in the video frame
    def backpropagate2frames(self, part_vid, start_idx, end_idx, random):
        part_vid.requires_grad = True      # retain the divided region gradient
        ...
        for idx, extractor in enumerate(self.extractors):
            perturb_loss = 0
            o = extractor(processed_vid)[0]   # get features
            o = o.view((o.size(0), -1))
            if self.target:
                if random:
                ...
                perturb_loss += nn.MSELoss(reduction='mean')(o, r)  # compute loss
            # print(perturb_loss.item())
        perturb_loss.backward()  # backpropagation
        extractor.zero_grad()    # update gradient
        sign_grad = torch.sign(part_vid.grad)  # get the gradient sign
        return sign_grad

    # get adversarial direction
    def create_adv_directions(self, vid, random=True):
        # format: [num_frames, c, w, h]
        vid = vid.clone().cuda(self.device)
        assert hasattr(self, 'target'), 'Error, AdvDirectionCreator\' mode unset'
        start_idx = 0
        adv_directions = []           # adversarial direction
        part_size = self.part_size    # partition size
        while start_idx < vid.size(0):
            # return a direction and update the block
            ...
            start_idx += part_size    # move index
